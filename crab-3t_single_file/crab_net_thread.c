		...
		if (seq == 0 || start_flag == 1){
			...
			if (mcnt_i < mcnt){
				if (miss_gap >=2){
					...
					//exit(1);
					miss_pkt +=(mcnt-mcnt_i)/512;
					fprintf(stderr,"misspkt:%ld\noffset:%ld\n",miss_pkt,offset);
					//exit(1);
					if ((offset + miss_pkt * N_CHAN_PER_PACK * N_POLS_CHAN) >= N_CHANS_BUFF){
						printf("\nProblem?\n");
						//exit(1);
						memset(db->block[block_idx].data+offset,0,(N_CHANS_BUFF-offset)*2*sizeof(char));
                                                db->block[block_idx].header.mcnt = mcnt_i;
                                                offset  = 0;    
						//start_flag = 0;
                                                mcnt_i += (N_CHANS_BUFF-offset)/4;
						// Mark block as full
				...
				memcpy(db->block[block_idx].data+offset,data0+8,DATA_SIZE_PACK*sizeof(char));
				db->block[block_idx].header.mcnt = mcnt_i;
				hashpipe_status_lock_safe(&st);
				hputi8(st.buf,"NETMCNT",mcnt);
