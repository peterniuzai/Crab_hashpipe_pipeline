...
/*	struct Full_Stokes{
	...
	hputi8(st.buf,"COV-MCNT",db_in->block[curblock_in].header.mcnt);
        ...
        hashpipe_status_unlock_safe(&st);
	//fprintf(stderr,"\n\nis there a problem??\n\n");
//	memcpy(packet_tmp,db_in->block[curblock_in].data,BUFF_SIZE*sizeof(char));
	//fprintf(stderr,"\n\nis there a problem?????????\n\n");
/*	for(int j=0;j<N_CHANS_SPEC*N_SPEC_BUFF;j++){
		xx = packet_tmp[j*4]/256;
		yy = packet_tmp[j*4+1]/256;
		I[j]=256*sqrt(pow(xx,2) + pow(yy,2))/82;		
		}

*/
	for(int j=0;j<N_CHANS_BUFF/N_POLS_CHAN;j++){
                //memcpy(&xx, db_in->block[curblock_in].data+j*8,  2*sizeof(char));
                //memcpy(&yy, db_in->block[curblock_in].data+j*8+2,2*sizeof(char));
//xx = block[curblock_in].data[j*8]
		I[j]=sqrt(pow(db_in->block[curblock_in].data[j*4],2) + pow(db_in->block[curblock_in].data[j*4+1],2));
//                I[j]=sqrt(pow(xx,2) + pow(yy,2));                
                }
